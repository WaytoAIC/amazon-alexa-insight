...
function extractAnswerFromAssistantSse(raw, question) {
  ...
  // 兜底：插件的旧启发式（与 content.js 逐行一致，差分测试锁住）
...
//
// 插件的 collectAssistantPatchText 把见过的每个 value.children 全收集起来，
// 只靠"后面有更长版本包含它"去重 —— 对 JSON Patch 语义是错的：中间态全被保留，
// 产出形如「Based on customer reviews for the [**Blink Subscription Plus /
// Based on customer reviews for the / , the most common complaints are:」的重复碎片。
//
// 正确做法是按序应用 patch 重建文档树，再按文档顺序取文本。
// 这是 CLI 与插件**有意分歧**的唯一一处；旧启发式保留为兜底。
...
module.exports = {
  ...
  extractAnswerFromJsonPatches,
  extractAnswerFromAssistantSseLegacy,
  applyJsonPatchStream,
  textFromNode,
  parsePointer,
